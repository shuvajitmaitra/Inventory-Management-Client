...
const ProductUpdate = () => {
  ...
  const { data: singleProduct, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(`/singleProduct/${params.id}`);
      return res?.data;
    },
  });
  if (isLoading) {
  ...
  const onSubmit = async (data) => {
    ...
    if (res?.data?.success) {
      ...
      const productPrice = Math.floor(
        makingCost + makingCost * 0.075 + makingCost * (profitMargin / 100)
      );
      ...
      const response = await axiosSecure.patch(
        `/productUpdate/${singleProduct._id}`,
        productData
      );
  ...
  return (
    <div className="min-h-screen flex justify-center items-center flex-col space-y-10 bg-zinc-100 ">
      ...
      <RouteTitle heading="Update Your Product!" />
